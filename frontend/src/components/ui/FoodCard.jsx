      ...
      <div className="p-5 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="w-full overflow-hidden">
            <p className="text-xs text-gray-500 flex items-start md:items-center gap-1 flex-row">
              <BiMap /> Near{" "}
              <span className="font-semibold truncate">
                {data?.place?.name}, {data?.city?.name}
        ...
        <div className="flex w-full justify-between items-center">
          ...
          <h1 className="flex justify-start items-center gap-2 truncate text-xs">
            {data?.category === "Veg" ? (
            ...
            {data?.specialFood?.map((i, index) => (
              <span key={index} className="bg-[#FFC20D] rounded-full py-1 px-2 font-medium">{i}</span>
            ))}
          </h1>
